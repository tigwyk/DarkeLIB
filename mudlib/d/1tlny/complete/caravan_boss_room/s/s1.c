...
if (!present("guard")) {
	  new("/d/1tlny/complete/caravan_boss_room/s/m1")->move(this_object());
	}
...
void create() {
    ...
    add_exit("/d/areas/arm_dealers/nalon/rooms/room31","south");    
    add_exit("/d/1tlny/complete/caravan_boss_room/s/s2.c","north");
/*    
set_items(([
     ({ "ceiling","beams" }) :
