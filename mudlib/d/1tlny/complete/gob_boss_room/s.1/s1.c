...
if (!present("guard")) {
	  new("/d/1tlny/complete/gob_boss_room/s.1/m1")->move(this_object());
	}
...
void create() {
    ...
    add_exit("/d/areas/goblin/areas/room1","south");    
    add_exit("/d/1tlny/complete/gob_boss_room/s.1/s2.c","north");
/*    
set_items(([
     ({ "ceiling","beams" }) :
