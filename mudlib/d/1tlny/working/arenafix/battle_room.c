 ...
 *  Modified by Vishnu for DarkeMUD
 */
...
void init() {
    ...
    add_action("no_thief", "use");
}

int no_thief(string str) {
	switch(str) {
		case "pick pocket" : message("arena", "%^RED%^You cannot do that in the arena.%^RESET%^", TP); return 1;
		case "steal" : message("arena", "%^RED%^You cannot do that in the arena.%^RESET%^", TP); return 1;
		case "disguise" : message("arena", "%^RED%^You cannot do that in the arena.%^RESET%^", TP); return 1;
	}
}
