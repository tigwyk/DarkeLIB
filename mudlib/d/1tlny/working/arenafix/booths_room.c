 ...
 *  Modified by Vishnu 6/27/97
 *  Copyright (c) 1996, DarkeMUD
 *  Do not remove this header
 */

#include <daemons.h>
#include "/d/damned/arena/arena.h"
...
private static object *preinv1, *preinv2;
...
void accept(object ob) {
    ...
    count = 0;
// added by Vishnu  //
    preinv1 = all_inventory(challenger);
    preinv2 = all_inventory(challengee);
// end of additions //
...
// Vishnu removes bugged eq
    all_inventory(challenger)->remove();
    all_inventory(challengee)->remove();

// Vishnu adds old eq
    preinv1->move(challenger);
    message("arena", "Old Equipment loaded", challenger);
    preinv2->move(challengee);
    message("arena", "Old Equipment loaded", challengee);
    call_out("move_all", 0, challenger, challenger_oldenv);
    call_out("move_all", 0, challengee, challengee_oldenv);

// Vishnu revives the simultaneous deaths (to fix simul-death bug, because someone is still dead)
    if(challenger->query_ghost()) {
 	challenger->revive();
	challenger->set_hp(challenger->query_max_hp());
	message("arena", "Thanks to Vishnu, you didn't die from Geldron's Bug", challenger);
         }
    if(challengee->query_ghost()) {
        challengee->revive();
        challengee->set_hp(challengee->query_max_hp());
        message("arena", "Thanks to Vishnu, you didn't die from Geldron's Bug", challengee);
         }
// I wasn't sure how to remove all poison, but this should about do it
    challengee->add_poisoning(-10000);
    challenger->add_poisoning(-10000);
