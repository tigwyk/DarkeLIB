...
void create() {
  ...
  set_long("This is the storage room for Schtel's shop.");
...
find_object_or_load("/wizards/gellor/newfirn/schtel_shop.c"));
  if(bob) {
      message("info", "Schtel says in Common: Finally! A new "
...
void finish_deliver(object bob) {
  if(bob)
      message("info", "Schtel says in Common: Now it's all here.",
            environment(bob));
  return;
} 
