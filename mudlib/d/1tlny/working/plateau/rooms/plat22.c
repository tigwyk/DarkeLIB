...
void create() {
   ...
   " The air has an eerie quality here, with spirits flitting around constantly"
   ", and you get the feeling they are urging you to %^RED%^jump off!!!"
    );
    set_exits( ({  "plat35.c", "plat23.c", "plat21.c" }),
      ({"north",  "west", "south" })
    );
...
int jump(string str)  {
   ...
   message("other action", player -> query_cap_name() + "%^RED%^ falls from above and lands at your feet in a pile with a sickening crunch.", PLATROOMS + "jump2.c", this_player());
}
   else {
   ...
   message("other action", player -> query_cap_name() + " floats gently down from above.", PLATROOMS + "jump2.c", this_player());
   return 1;
}}
}
