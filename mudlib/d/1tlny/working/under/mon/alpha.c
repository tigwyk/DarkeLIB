...
int zlevel,xskill,roll,xchange,xname;
	...
	set("aggressive", (: call_other, this_object(), "check_it" :));
	set_level(zlevel+3);
	xskill=7*level+20;
...
int check_it, race_check, attack_drill, sign_filter;
...
int attack_drill(object who){
	...
	"\nThe drow warrior shouts,\n"+
	"You don't belong here intruder!\n",
	({this_object()});
	return 1;
}


int race_check(object who){
	int gld;
	string p_name;
	object join_room, *inv;
inv = filter_array(all_inventory(this_player()), "sign_filter", this_object());
...
int check_it(object who){
	if(!who->is_player()) return 0;
	call_out("race_check", 1, who);
	return 1;
}
