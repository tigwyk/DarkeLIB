...
void create() {
	::create();
        set_name("waterfall");
        set_id(({ "waterfall", "water" }) );
        set_short("%^BOLD%^%^RED%^large mounds of rotting flesh%^MAGENTA%^ and%^CYAN%^%^BOLD%^ a sparkling waterfall%^MAGENTA%^%^BOLD%^");
	set_long(
"The sparkling water fall flows evenly over the rocks. Splashing water "
"everywhere, the nearby rocks are kept clean of the blood stains "
...
void init() {
  	...
  	add_action("fill_vessel", "fill");
        ...
        add_action("drink_waterfall", "drink");
...
void story_telling() {
	...
	string story;
        switch(random(2)) {
			...
			break;
                case 1: story = "\nA large shadow moves about behind the "
"falls.\n";
			break;
	}
