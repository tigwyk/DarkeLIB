...
void skill_func(object from, object at, string arg) {
    ...
    if((time() - (int)from->query_last_use("focus of elements")) < (6000 /
...
void do_focus(object from, string arg) {
    ...
    sk_bonus = 60 + random(20) + random(20);
    message("info", sprintf("%^CYAN%^Your talent in the lore of %s increases by %d%%!%^RESET%^",
	sk_bonus, arg), from);
    arg += " lore";
