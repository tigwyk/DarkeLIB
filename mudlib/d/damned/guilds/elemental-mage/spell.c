...
inherit OBJECT;
inherit "/std/spells/spell_hooks";
...
int not_wizard(object who){
    if(wizardp(who) && !who->query_property("masochist") || who->query_property("no kill"))
	return 0;
    return 1;
}
...
void do_criticals(string *criticals, string target_thing, object caster, object targ) {
	...
	if(!tmp || !sizeof(tmp)) continue;
		...
		case "LIMB_LOSS":
			...
			targ->cripple_limb(what, "cripple");
			targ->check_on_limb(what);
		    } else if(targ) {
			targ->cripple_limb(what, "sever");
			k = targ->query_property("bleeding");
			targ->set_property("bleeding", k + 25);
		    }
		    break;
		case "STAT_MOD":
		...
		case "SKILL_MOD":
		    ...
		    call_out("remove_skill_mod",dur*2,([ "who" : targ,
...
void do_base_criticals(string *criticals, string target_thing, object caster, object targ) {
    int i,j, amt, roll, dur;
    string *tmp, what, what2, *tmp2, *mesgs;
    object ob;
    object *targ_armour;
...
void begin_casting(object caster, string arg, int power) {
    ...
    string xtra_arg, target, ele, nospellmsg;
    int i, which;
    object *inv;
	...
	if(!nospellmsg = environment(caster)->query_property("no spell msg"))
	    nospellmsg = "Mystic forces prevent casting.";
	message("info", nospellmsg + "\n", caster);
	remove();
	return;
    }
...
/* Edit for self casting spells added by Cythug 9/19/02 */
if(props["self cast"] && arg != caster->query_name()) {
message("info", "You may only cast this spell at yourself.", caster);
remove();
return;
}
    ...
    */
	...
	message("info","%^GREEN%^" +caster->query_cap_name()+
	  " begins casting a ritual spell with "+caster->query_possessive()+" group.",           environment(caster), ({caster}));
    }
    ...
    inv = filter_array(inv, "living_filter");
    for(i = 0; i < sizeof(inv); i++)
	inv[i]->spell_cast(props["name"], props["skill"], caster);
	...
	while(i-- && !caster->query_property("no spell bonus")) {
    ...
    // mass casting modifications
    if(call_value["target"] == "everyone" 
      && query_property("skill") != "tmp skill"){
	props["casting time"] *= 8;
	props["base mp cost"] *= 4;
    }
...
void set_element() {
	...
	break;
    case "entropy":
	props["element types"] = ({ "acid", "radiation", "stress" });
	break;
    case "flash":
	props["element types"] = ({ "time", "reflection", "electricity" });
	break;
    }
    if(props["ele damage"]) {
...
void generate_observe_messages(string atstr, object at, object caster, int masscast){
	...
	props["caster message"] = replace_string(props["caster message"],"$T",
	  capitalize((string)at->query_name()));
	props["observer message"] = replace_string(props["observer message"],"$T",
...
int no_arch(object x){
    if(wizardp(x))return 0;
    return 1;
}
...
void do_spell(mapping info) {
    ...
    int power, roll, fumble_flag, resist, i, j;
    int mp_cost;
    object leader;
    int fumble_shield;
    int masscast = 0;
	...
	message("info","Your spell fails because you are stunned.",caster);
	remove();
	return;
    }
	...
	case "all":
	    if(masscast)
		targets = filter_array(users(), "no_arch", this_object());
	    else {
		if(!props["must be present"])
	...
	if(!masscast && at->query_invis() && !caster->query("see invis") && at != caster) at = 0;
	if(!at && !masscast) { message("info","Can't find "+target+" here!  "
    ...
    roll = random(100) + 1;
    fumble_shield = (int)caster->query_skill("fumble shield");
    if(fumble_shield > 90)fumble_shield = 90;
    if(!masscast)
	targets = ({ at });
	...
	generate_observe_messages(target, at, caster, masscast);

	if(roll > (94 + skill / 34) || roll == 100) {

	    if(!caster->query("no fumble") && !caster->query_property("no fumble")) {
		if (random(100)+1 < fumble_shield)
		    message("info",
		      "%^RED%^%^BOLD%^You start to fumble your spell, but fumble shield saves you."
		      ,caster);
		else{
		    message("info","%^RED%^%^BOLD%^You fumble your spell!",caster);
		    message("info","%^RED%^%^BOLD%^"+(string)caster->query_cap_name()+
		      " fumbles "+possessive(caster)+" spell!", environment(caster),({ caster }));
		    fumble_flag = 1;
		}
	    } }
	else if(roll > skill) {
		...
		if(!caster->kill_ob(at,0)) return;
	    caster->add_exp2(13*props["spell level"]*SPELL_CAST_EXP_MOD);
	    remove();
	    return;
	}
	...
	caster->add_exp2(25 * props["spell level"]*SPELL_CAST_EXP_MOD);
    ...
    if(props["target type"] == "all")remove();
    return;
}

void spell_func(object caster, object at, int power, string args, int flag) {
    int roll, i, j, k, l,dice, tmp_dice, sides, damage_this_hit;
    int resist;
    int tmpdam;
	...
	if(props["spell type"][i] == "area damage" &&
	  !flag)
		...
		environment(caster)),"not_party", this_object(),caster);
	else if(!flag)
	    inv = ({ at });
	...
	while(k--) {
		...
		tmpdam = (props["damage types"][tmp[j]] * (power + 2)/2);

		if(skill > 100)
		    tmpdam = scale(tmpdam, skill, 100);

		res = (string)DAMAGE_D->attack_result(sprintf("%d %d %d",
		    random(100)+1, tmpdam / (resist_flag?2:1), (int)inv[k]->
	...
	while(j--) {
		...
		//                 extra = (: extra_bonus, roll :);
		// new hooks implementation by Thrace Merin
		add_bonus(at, props["stats"][j], roll);
	...
	call_out("remove_bonus2", props["duration"], at);
	break;
    case "skill mod":
	...
	while(j--) {
		...
		roll /= resist_flag?2:1;
		add_bonus(at, props["skills"][j], roll);
	    }
	}
	call_out("remove_bonus2", props["duration"], at);
	break;
	...
	ob = ({ clone_object("/std/spells/shadows/ac_shadow") });
	if(props["stack key"])
	    ob[0]->set_stack_key(props["stack key"]);
	ob[0]->set_shadow_name(props["name"] + " (AC)");
	ob[0]->start_shadow(at,props["duration"]);
	j = sizeof((tmp = keys(props["protection types"])));
	while(j--)
	    if(member_array(tmp[j],DAMAGE_TYPES) > -1){
		tmpdam = props["protection types"][tmp[j]]*power / (resist_flag?2:1);

		if(skill > 100)
		    tmpdam = scale(tmpdam, skill, 100);

		ob[0]->set_shad_ac(tmpdam,tmp[j]);
	    }
	ob[0]->set_expire_message("%^CYAN%^A protection spell wears off.");
	break;	    
    }
    if(damage_this_hit > (int)caster->query_statistic("big spell hit"))
	caster->set_statistic("big spell hit", damage_this_hit);
