...
if(wizardp(this_player())) return 0;
	...
	if(find_player(p_name) && find_player(p_name)->query_class() == "fighter") return 0;
	     force_me("speak common");
     force_me("say I will not allow a pet named "+TP->query_name()+" from entering as you are not a Fighter");		
		return 1;
    }
  ...
  if(wizardp(this_player())) return 0;
   if((string)this_player()->query_class() != "fighter") {
    force_me("speak common");
     force_me("say Only fighters may pass");
