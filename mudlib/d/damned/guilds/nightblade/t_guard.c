...
int block_exit() {
  ...
  object join_room, *inv;
	...
	if(find_player(p_name) && find_player(p_name)->query_class() == "nightblade") return 0;
	     force_me("speak common");
     force_me("say I will not allow a pet named "+TP->query_name()+" from entering as you are not a Nightblade");		
		return 1;
    }
  ...
  if((string)this_player()->query_class() != "nightblade") {
	     force_me("speak common");
     force_me("say Only nightblades may pass.");
    return 1;
  }
