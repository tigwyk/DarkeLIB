...
int block_exit() {
  ...
  object join_room, *inv;
	...
	if(find_player(p_name) && find_player(p_name)->query_class() == "war-priest") return 0;
	     force_me("speak common");
     force_me("say I will not allow a pet named "+TP->query_name()+" from entering as you are not a War-Priest");		
		return 1;
    }
  ...
  if((string)this_player()->query_class() != "war-priest") {
	     force_me("speak common");
     force_me("say Only war-priests may pass.");
    return 1;
  }
