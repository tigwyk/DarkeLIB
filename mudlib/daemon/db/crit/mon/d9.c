...
void create() {
  ::create();
  set_name("elemental");
  set_id(({ "elemental", " greater elemental", "inertia elemental" }));
  set("race", "elemental");
  set_body_type("human");
  ...
  set_level(20);
  set_overall_ac(20);
  set_short(" greater inertia elemental");
  set_long("This greater elemental glows with the energy of inertia");
  set_aggr_status(0);

	set_melee_damage( (["crushing" : 20 , "stress" : 20, "impact" : 20 ]) );
    ...
    set_stats("intelligence", 110 + random(20));
...
string query_element() { return "inertia"; }
...
void remove_servant() {
  ...
  message("info", "%^GREEN%^%^BOLD%^A Greater elemental winks out of existence.",
	  environment());
  remove();
  return;
}
