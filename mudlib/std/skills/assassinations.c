...
void skill_func(object from, object at, string arg) {
    ...
    if((time() - (float)from->query_last_use("assassinations")) <= 4.100000) {
	message("info", "You are already attempting an assassinations at this time.", from);
	remove();
	return;
    }
    ...
    skill = (int)from->query_skill("assassinations");
    ...
    difference = to_int(to_float((int)from->query_skill("assassinations")) / to_float(2));
    ...
    if(((int)at->query_skill("reverse stroke") > difference) && ((int)at->query_skill("reverse stroke") < (int)from->query_skill("assassinations"))) {
	if(randomizer <= value) {
	    hit = 2;
	}
	if(randomizer > value) {
    ...
    if((int)at->query_skill("reverse stroke") >= (int)from->query_skill("assassinations")) {
	hit = 2;
    }
    from->set_last_use("assassinations");
