...
void create() {
    ::create();
    set_property("name", "extreme elemental weapon");
    set_property("skill", "elemental magic");
    set_property("casting time", 27);
    set_property("dev cost", 272);
    set_property("fast dev cost", 888);
    set_property("spell level", 38);
    set_property("moon", "warzau");
    set_property("caster message", "You cast %^BOLD%^%^CYAN%^extreme elemental weapon%^RESET%^ at $T.");
    set_property("target message", "");
    set_property("observer message", "$C casts %^BOLD%^%^CYAN%^extreme elemental weapon%^RESET%^ at $T.");
...
void info() {
    message("help", "Energy mass units required for spell: 125\n\nThis is an incredibly powerful magic that enables an Elemental Mage to imbue extremely potent elemental power into any weapon, significantly increasing its physical damage and critical hits damage to very high levels in the element(s) drawn directly from the casters energy massThis spell requires energy units drawn from a mass to cast, not mp. The element(s) this spell will contain is determined by the element(s) present in the mass.\n\nSyntax: 'cast *6 extreme elemental weapon at staff'", this_player());
}
...
void spell_func(object caster, object at, int power, string args, int flag) {
	...
	if(inv[i]->query_property("owner name") == (string)caster->query_name()) {
	    if(inv[i]->query_property("energy units") >= 125) {
		inv[i]->set_property("energy units", ((inv[i]->query_property("energy units")) - 125));
		...
		if(at->query_property("stack count") < 3) {
			...
			ob->set_extra_wc(([ ""+lorename1+"" : 21*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
		    }
		    if(((string)lorename2 != 0) && ((string)lorename3 == 0) && ((string)lorename4 == 0)) {
			ob->set_extra_wc(([ ""+lorename1+"" : 21*power, ""+lorename2+"" : 21*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename2+"%^RESET%^!", caster);
		    }
		    if(((string)lorename2 != 0) && ((string)lorename3 != 0) && ((string)lorename4 == 0)) {
			ob->set_extra_wc(([ ""+lorename1+"" : 21*power, ""+lorename2+"" : 21*power, ""+lorename3+"" : 21*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename2+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename3+"%^RESET%^!", caster);
		    }
		    if(((string)lorename2 != 0) && ((string)lorename3 != 0) && ((string)lorename4 != 0)) {
			ob->set_extra_wc(([ ""+lorename1+"" : 21*power, ""+lorename2+"" : 21*power, ""+lorename3+"" : 21*power, ""+lorename4+"" : 21*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename2+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename3+"%^RESET%^!", caster);
		    ...
		    message("info", "%^RED%^You expend %^RESET%^125 %^RED%^energy units to cast the spell.%^RESET%^", caster);
