...
void create() {
    ::create();
    set_property("name", "major elemental weapon");
    set_property("skill", "elemental magic");
    set_property("casting time", 21);
    set_property("dev cost", 129);
    set_property("fast dev cost", 402);
    set_property("spell level", 23);
    set_property("moon", "warzau");
    set_property("caster message", "You cast %^BOLD%^%^CYAN%^major elemental weapon%^RESET%^ at $T.");
    set_property("target message", "");
    set_property("observer message", "$C casts %^BOLD%^%^CYAN%^major elemental weapon%^RESET%^ at $T.");
...
void info() {
    message("help", "Energy mass units required for spell: 40\n\nThis more powerful version of the spell elemental weapon permits an Elemental Mage to imbue a given weapon with considerably strong elemental magic, which will increase its damage both with physical and critical hit damage to a very notable degree in the element(s) derived from the energy mass the power was drawn from. This spell requires energy units drawn from a mass to cast, not mp. The element(s) this spell will contain is determined by the element(s) present in the mass.\n\nSyntax: 'cast *6 major elemental weapon at staff'", this_player());
}
...
void spell_func(object caster, object at, int power, string args, int flag) {
	...
	if(inv[i]->query_property("owner name") == (string)caster->query_name()) {
	    if(inv[i]->query_property("energy units") >= 40) {
		inv[i]->set_property("energy units", ((inv[i]->query_property("energy units")) - 40));
		...
		if(at->query_property("stack count") < 3) {
			...
			ob->set_extra_wc(([ ""+lorename1+"" : 15*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
		    }
		    if(((string)lorename2 != 0) && ((string)lorename3 == 0) && ((string)lorename4 == 0)) {
			ob->set_extra_wc(([ ""+lorename1+"" : 15*power, ""+lorename2+"" : 15*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename2+"%^RESET%^!", caster);
		    }
		    if(((string)lorename2 != 0) && ((string)lorename3 != 0) && ((string)lorename4 == 0)) {
			ob->set_extra_wc(([ ""+lorename1+"" : 15*power, ""+lorename2+"" : 15*power, ""+lorename3+"" : 15*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename2+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename3+"%^RESET%^!", caster);
		    }
		    if(((string)lorename2 != 0) && ((string)lorename3 != 0) && ((string)lorename4 != 0)) {
			ob->set_extra_wc(([ ""+lorename1+"" : 15*power, ""+lorename2+"" : 15*power, ""+lorename3+"" : 15*power, ""+lorename4+"" : 15*power ]));
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename1+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename2+"%^RESET%^!", caster);
			message("info", "You imbue the weapon with %^BOLD%^%^YELLOW%^"+(string)lorename3+"%^RESET%^!", caster);
		    ...
		    message("info", "%^RED%^You expend %^RESET%^40 %^RED%^energy units to cast the spell.%^RESET%^", caster);
