...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  if((int)at->query_level() < 29/*query_property("spell level")*/) {
 ...
 // start_work(at, caster, 8000+560*power, power);
 start_work(at, caster, 50, power);
  return;
}
...
void finish_work(object caster, object at, int power) {
  ...
  at->remove();
  seteuid(getuid());
  ob = new("/wizards/honspron/pets/nethergeist");
  ob->set_up(power);
