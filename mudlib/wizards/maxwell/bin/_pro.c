...
int cmd_pro(string str) {
  ...
  if(target != this_player() && !wizardp(this_player())){
    write("You cannot examine that player's protection.");
    return 1;
  }
  limbs = target->query_limbs();
  if(!limbs || !(i=sizeof(limbs)) || target->query_ghost()) {
  ...
  if(!str || str != "limb") {
    ...
    while(j--) {
      ...
      i = sizeof(limbs);
      while(i--)
        ac += (int)target->query_current_protection(limbs[i], DAMAGE_TYPES[j]);
      ac /= sizeof(limbs);
      tmp += "%^BLUE%^|%^RESET%^ ";
    ...
    while(i--) {
      ...
      while(j--) {
        if(member_array(DAMAGE_TYPES[j], PHYSICAL) < 0)
          ...
          p_ac += (int)target->query_current_protection(limbs[i], DAMAGE_TYPES[j]);
      }
