...
int train_skill(string str,object target) {
  int cost, my_dev, my_lev;
 
  if(!str || !target) return 0;
  if(!present("trainer")){
     write("The trainer isn't here!");
     return 1;
  }
  my_dev = (int)target->query_property("dev points");
  my_lev = (int)target->query_level();
  str = lower_case(str);
  if(!str) return 0;
  if(!skill_list[str]) {
    tell_object(target,"There is no such skill in this guild.");
    return 1;
  }
  if(skill_list[str]["level"] > my_lev) {
    tell_object(target,"That skill is not available at your level.");
    return 1;
  }
  cost = dev_cost(target, str);
  if(cost > my_dev) {
    tell_object(target,"You need "+cost+" development points to train that skill.\n"+
	  "You only have "+my_dev+".");
    return 1;
  }
  target->add_dev(-1 * cost);
  target->set_skill(str, (int)target->query_base_skill(str) +
			   calc_skill_inc(target, str));
  target->set_last_dev(str, my_lev);
  tell_object(target,"You now have "+str+" at "+(int)target->query_skill(str)+"%");
  tell_object(target,"It will cost you "+dev_cost(target, str)+
        " development points to advance this skill further.");
  tell_object(target,"You have "+sprintf("%d", (my_dev - cost)) + 
