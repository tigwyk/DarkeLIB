...
int z,loc,number,num3,temp,count,nnum;
mixed counter;
...
//Second Series
    ...
    rm("/wizards/maxwell/objects/etc/output6.c");
    z=0;
    rrp=0;
    counter = allocate(1000);
    numlist = allocate(1000);
    temp = 0;
    while(z<10000){
    ...
    z = 0;
    while(z<sizeof(numlist)){
      if(numlist[z] != 0 || (z>870 && z<930)){
        write_file(UNETC+"output5.c",(z/100.0)+"\n");
        write_file(UNETC+"output6.c",numlist[z]+"\n");
      }
      z = z + 1;
    }
    write("Done.");
