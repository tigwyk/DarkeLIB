...
ob = new("std/bboard");
  ob->set_name("board");
  ob->set_id( ({"board", "mine_board"}) );
  ob->set_board_id("mine_board");
  ob->set_max_posts(25);
  ob->set_location(UNDER+"dmc");
  ob->set("short","Under'Drin Mines Board");
  ob->set("long","A sturdy looking message board carved from some high quality timber.\n");
   ob->set_edit_ok(({wizardp(this_player())}) );
...
"A large sign has been bolted to the gate.\n"
    ...
    set_exits( ({ "/d/damned/virtual/room_35_13.world",UNDER+"dme"}),
      ({ "mountains","entrance"})
